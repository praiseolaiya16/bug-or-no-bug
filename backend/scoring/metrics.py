...
def score_by_bug_type(
    ...
    """
    bug_types = sorted({bug["bug_type"] for bug in ground_truth if bug.get("is_bug")})

    scores = {}
    for bug_type in bug_types:
        subset_ground_truth = [b for b in ground_truth if b.get("bug_type") == bug_type]
        subset_files = {b["file"] for b in subset_ground_truth}
        subset_predicted = [f for f in predicted if f.get("file") in subset_files]
        scores[bug_type] = precision_recall_f1(subset_predicted, subset_ground_truth)
